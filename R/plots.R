...
interactionplot <- function(x, ...) {
  if (!is.matrix(x)) {
    ...
    matplot(t(x), type = "b", xaxt = "n", ylab = "Observed values", xlab = "Factor1(column)", col = topo.colors(b), lwd = 2, lty = 1:b, ...)
    ...
    matplot(x, type = "b", xaxt = "n", ylab = "", xlab = "Factor2(row)", col = topo.colors(t), lwd = 2, lty = 1:t, ...)
